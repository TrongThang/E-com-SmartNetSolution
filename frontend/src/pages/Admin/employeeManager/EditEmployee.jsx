...
export default function EditEmployeeForm() {
    ...
    const validateForm = () => {
        ...
        // Only validate image if no image exists and none is uploaded
    ...
    const fetchEmployeeData = async () => {
        try {
            ...
            if (res.status_code === 200) {
                const employee = res.data.data[0] // Access the first (and only) employee object
                setFormData({
                ...
                Swal.fire({
                    icon: "error",
                    title: "Lỗi",
                    text: "Không thể tải thông tin nhân viên. Vui lòng thử lại.",
                    confirmButtonText: "Đóng",
                })
            }
        } catch (err) {
            ...
            Swal.fire({
                icon: "error",
                title: "Lỗi",
                text: "Có lỗi xảy ra khi tải thông tin nhân viên. Vui lòng thử lại.",
                confirmButtonText: "Đóng",
            })
        }
    }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...
                gender: formData.gender ? true : false, // Map   true (male), false (female) for the API
                status: formData.status,
                username: formData.username,
                role: formData.role, // Send the role ID
                image: formData.image,
            }
            console.log("Submitting payload:", payload)
            const res = await EmployeeApi.edit(String(id), payload)
            if (res.status_code === 200) {
                Swal.fire({
                    icon: "success",
                    title: "Thành công",
        ...
        <div className="min-h-screen p-6">
            <div className="">
                <div className="mb-6">
                    ...
                    <h1 className="text-3xl font-bold text-gray-900">Chỉnh sửa thông tin nhân viên</h1>
                </div>
